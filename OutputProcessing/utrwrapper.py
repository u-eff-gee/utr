...
"""
    + programName
    + """ reads this header and based on it prepares the simulation
...
#                                     # (Default: True)
#ensureTerminalMultiplexer=False      # Whether to warn and abort if """
    + programName
    + """
...
argparser.add_argument(
    "macFile", metavar="EXTENDEDMACROFILE", help="the extended macro file to process"
)
argparser.add_argument(
    "-s",
    "--skipSimulation",
...
if not os.path.isfile(macFile):
    exit(
        "ERROR: Supplied extended macro file path'"
        + macFile
        + "' is no valid file! Aborting..."
    )
...
with open(macFile) as openMacFile:
    for line in openMacFile:
        ...
        # All other lines in the configuration part are forbidden
            ...
            exit(
                "ERROR: Malformed extended macro file: Encountered non-empty line '\n"
                + line
                + "' without a leading '#' in the configuration header of the extended macro file (before '#START_OF_MACRO' line)! Aborting..."
            )
...
)  # Keep keys' case as is, overwrites the default of converting keys to lowercase
config.read_string("".join(cfgParts))
if not config.has_section("generalConfig"):
    exit(
        "ERROR: Extended macro file is missing required section '[generalConfig]' in its configuration header"
    )

# If required by config, script will abort if not run in a TMUX or Screen session
if (
    config["generalConfig"].getboolean("ensureTerminalMultiplexer", False)
    and "TMUX" not in os.environ
    and "STY" not in os.environ
):
    exit(
        "ERROR: "
        + os.path.basename(sys.argv[0])
        + " is not run within a tmux or screen session as required by the configuration header! Aborting..."
    )
...
utrPath = os.path.realpath(os.path.join(os.path.dirname(__file__), ".."))
if utrPath.endswith("/build"):
    utrPath = utrPath[:-6]
utrPath = config["generalConfig"].get("utrPath", utrPath)
checkForExistingOutput = config["generalConfig"].getboolean(
    "checkForExistingOutput", True
)
...
if processOutput or checkForExistingOutput or logging:
    if (
        "environmentVariables" not in config
        or "filenamePrefix" not in config["environmentVariables"]
        or "filenameSuffix" not in config["environmentVariables"]
    ):
        exit(
            "ERROR: To process the output, write a log, or check for already existing output the keys 'filenamePrefix' and 'environmentVariables' must be set in the '[environmentVariables]' section of the configuration header in the extended macro file! Aborting..."
        )
...
if (
    ...
    exit(
        "ERROR: Output directory seems to contain files matching the supplied filename structure! Aborting..."
    )
...
def error(*args, **kwargs):
    ...
    loggingPrint(
        *(args + tuple(["Aborting..."])), tag="ERROR:", file=sys.stderr, **kwargs
    )
    exit(1)
...
loggingPrint(
    "Started processing extended macro file '" + macFile + "' at",
    startTime.strftime("%Y-%m-%d %H-%M-%S"),
)
if args.skipSimulation:
    loggingPrint(
        "Command-line argument '--skipSimulation' was supplied. Simulation will be skipped!"
    )
            ...
            with open(mac) as openMac:
                ...
                for line in openMac:
                    ...
                    for (cmd, pos) in [
                        ...
                        if (
                            splitLine[0] == "/control/" + cmd
                            and splitLine[pos] not in fileQueue
                        ):
...
def runProcess(prog, procArgs, announce=True, **kwargs):
    if announce:
        ...
        loggingPrint(
            ...
            f"(current runtime: {elapsedSeconds//(60*60)}h {(elapsedSeconds//60)%60:02}m {elapsedSeconds%60:02}s)",
        )
    if not logging:
...
if not args.skipSimulation:
    runProcess(
        "cmake on utr", ["cmake", "-S", ".", "-B", "build"] + cmakeArgs, cwd=utrPath
    )
    runProcess(
        "buildsystem on utr",
        ["cmake", "--build", "build", "-j", str(threads)],
        cwd=utrPath,
    )
if processOutput:
    ...
    if "getHistogramArgs" in config:
        for opt in config["getHistogramArgs"]:
            if config["getHistogramArgs"][opt] == "":
                ...
                getHistogramArgs.append(
                    "--" + opt + "=" + config["getHistogramArgs"][opt]
                )
    ...
    if "histogramToTxtArgs" in config:
        for opt in config["histogramToTxtArgs"]:
            if config["histogramToTxtArgs"][opt] == "":
                ...
                histogramToTxtArgs.append(
                    "--" + opt + "=" + config["histogramToTxtArgs"][opt]
                )
    runProcess(
        "cmake on OutputProcessing scripts",
        ["cmake", "-S", "OutputProcessing", "-B", "build/OutputProcessing"],
        cwd=utrPath,
    )
    runProcess(
        "buildsystem on OutputProcessing scripts",
        ["cmake", "--build", "build/OutputProcessing", "-j", str(threads)],
        cwd=utrPath,
    )
...
if processOutput:
    ...
    for rootFilename in outputFiles:
        loggingPrint(
            "Processing output file '" + rootFilename.rpartition("_t")[0] + "'..."
        )
...
elapsedSeconds = int((currentTime - startTime).total_seconds())
loggingPrint(
    "Finished processing extended macro file '" + macFile + "' at",
    currentTime.strftime("%Y-%m-%d %H-%M-%S"),
    f"(took {elapsedSeconds//(60*60)}h {(elapsedSeconds//60)%60:02}m {elapsedSeconds%60:02}s)",
)
