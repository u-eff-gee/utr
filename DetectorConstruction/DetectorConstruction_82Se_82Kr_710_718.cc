...
G4VPhysicalVolume *DetectorConstruction::Construct() {
	...
	// WALL7: Lead + concrete shielding of 2nd setup (82Se/82Kr experiment runs 710 - 760)
	//
	...
	G4double SecondTarget_AngleX = -22.*deg; // Rotation angle to have the valve rest on the inside of the TargetTube
	rotateSecondTarget->rotateX(SecondTarget_AngleX);

	new G4PVPlacement(
	    rotateSecondTarget,
	    G4ThreeVector(0., 20.*mm*sin(SecondTarget_AngleX), -BeamTube_Length_Downstream +
	                              BeamTube_Length * 0.5 + Target2_To_Target -
	                              kr82_Target->Get_Target_Center() - 20.*mm*(1-cos(SecondTarget_AngleX))),
...
//	AuxiliaryBox_Logical->SetVisAttributes(new G4VisAttributes(white));
	...
	pbthinsmall->Put(HPGe3_rt * sin(HPGe3_theta) *
	                    cos(HPGe3_phi),
	                HPGe3_rt * sin(HPGe3_theta) *
	                        sin(HPGe3_phi) +
	                    HPGe3_dy,
	...
	HPGe3_rt -= fc->FilterCaseBottom_Thickness - fc->FilterCase_Length * 0.5;
	fc->Put(HPGe3_rt * sin(HPGe3_theta) * cos(HPGe3_phi),
	        HPGe3_rt * sin(HPGe3_theta) * sin(HPGe3_phi) + HPGe3_dy,
	        HPGe3_rt * cos(HPGe3_theta) + HPGe3_dz, HPGe3_AngleX, HPGe3_AngleY,
	        0.);
...
*/
	...
	*/
	...
	G4double Germanium2_TUD_theta = 93. * deg; // Detector was visibly tilted in the experiment
	...
	G4Tubs *Polarimeter_TUD_Wrapping_Solid = new G4Tubs(
	    "Polarimeter_TUD_Wrapping_Solid", Polarimeter_TUD_Instance->Get_Radius(),
	...
	cuthin50->Put(
	    Polarimeter_TUD_rt * sin(Polarimeter_TUD_theta) * cos(Polarimeter_TUD_phi),
	    Polarimeter_TUD_rt * sin(Polarimeter_TUD_theta) *
	            sin(Polarimeter_TUD_phi) +
	        Polarimeter_TUD_dy,
	    Target2_To_Target + Polarimeter_TUD_rt * cos(Polarimeter_TUD_theta) +
	        Polarimeter_TUD_dz,
