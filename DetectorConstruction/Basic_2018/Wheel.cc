...
Wheel::Wheel(){
	...
	Wheel_Max_Radius = 0.5*34.*inch;
	double 	Hole_Radius = 0.5*16.*inch;
...
// Ring numbers go from upstream to downstream
	...
	std::vector<G4double> Ring_Thickness = {0.75*inch, 0.5*inch, 0.5*inch, 1.*inch, 0.5*inch, 0.75*inch, 1.*inch};
	...
	G4Tubs *Wheel_Solid = new G4Tubs("Wheel_Solid", 0., Wheel_Max_Radius, Wheel_Thickness*0.5, 0., twopi);
	
	Wheel_Logical = new G4LogicalVolume(Wheel_Solid, Al, "Wheel_Logical");
	//Wheel_Logical->SetVisAttributes(G4VisAttributes::GetInvisible());
	...
	for(unsigned int i = 0; i < Ring_Radius.size(); ++i){
		...
		new G4PVPlacement(0, G4ThreeVector(0., 0., pos_z), Wheel_Ring_Logical, physicalName.str(), Wheel_Logical, false, 0, false);
		pos_z += Ring_Thickness[i]*0.5;
