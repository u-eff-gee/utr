...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    nb->Put(-nb->L + Wall7_Width * 0.5 + nb->L * 0.5,
            ...
            Wall7_To_Target + nb->S + nb->M * 0.5, 0., 90. * deg, 0.);
    if (i > 1) {
      snb->Put(0., BeamTube_Outer_Radius - hsbh->Hole_Radius + nb->S * (0.5 - i),
  ...
  bb->Put(0., bb->M * 0.5, Wall7_To_Target + nb->S * 3. + PipeHolderTubeUpStream_Length + bb->S * 1.5,
          -90. * deg, 90. * deg, 0.);
  bb->Put(0., bb->M * 0.5, Wall7_To_Target + nb->S * 3. + PipeHolderTubeUpStream_Length + bb->S * 2.5,
          -90. * deg, 90. * deg, 0.);
