...
void G3_Wall::Construct(G4ThreeVector global_coordinates){
	...
	new G4PVPlacement(
	    0, G4ThreeVector(global_coordinates.x(), global_coordinates.y()-BeamTube_Outer_Radius - nb->S * 3. - cb->M -
	                             AlPlate_Y * 0.5,
	                     global_coordinates.z() - nb->M * 3 + AlPlate_Z * 0.5),
	    AlPlate_Logical, "AlPlate", World_Logical, false, 0);

	// Concrete Blocks below aluminium plate

	fcb->Put(global_coordinates.x(), global_coordinates.y()-BeamTube_Outer_Radius - nb->S * 3. - cb->M - AlPlate_Y -
	                 fcb->M * 0.5,
	         global_coordinates.z() - nb->M * 3 + fcb->L * 0.5, 0., 0., 90. * deg);
	cb->Put(global_coordinates.x()-fcb->S * 0.5 - cb->M * 0.5, global_coordinates.y()-BeamTube_Outer_Radius - nb->S * 3. -
